...
class Bank
{
	...
	int balance;
public:
	void AccountCreation();   //To Create a new account
	void showAccount();     //To display an account details
...
void Bank::AccountCreation()
{
	...
	cin.ignore();                   // Ignore or clear one or more characters from the input buffer.
	cin.getline(accountType,20);   //To take only first 50 characters as the account type entered by the user
...
void Bank::report()
{
	cout<<account_no<<setw(10)<<" "<<name<<setw(10)<<" "<<accountType<<setw(6)<<balance<<endl;     //Specifies the minimum number of character positions a variable will consume
}
